...
checks = [
    ...
    # 「进化审计」整页（判据强度 / 候选删除 / 待补判据 / 断头双份 + 批量删除按钮）
    # 删了，原来守它的两条断言一并删。⛔ 删的理由不是「不重要」，是**不该由用户判**：
    # 他既判不了「这条判据够不够结构性」，也不想判；而一个摆着「删除」按钮的清单
    # 只会让他担心自己的文件会不会没了。
    # 真正要保留的能力只有一个——让某一条坑退休，它现在长在坑库那一行上。
    ("坑能就地退休（在坑库那一行，不在别的页面勾清单）", "pit-retire" in appjs),
    ("退休的措辞不吓人（不写「删除」）", "这条不用了" in appjs),
    ("退休时说清只动这一行", "你的项目和文件一个都不动" in appjs),
]
for n, ok in checks:
